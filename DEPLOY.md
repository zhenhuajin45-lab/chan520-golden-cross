...
第二轮实战版回测默认使用共享账户组合引擎，不是逐票独立相加。默认纪律包括：

- 单笔风险 1%、单股 20%、单行业 40%。
- 市场 regime 总仓位：`trend_up=80%`、`range=50%`、`down=30%`。
- 现金保留 20%。
- 只接受标准 520 入选，执行“四不做”和 `R:R>=2`。
- D+1 开盘成交前复核涨跌停、现金、单股、行业、总仓位。
- 持仓后执行移动止损、时间止损、3 天不创新高和跌破 MA20 退出。

复现第二轮证据：

```powershell
python -m chan520_skill backtest --basket "600288,300568,688106,002132,603638,301282,600203,300390,002830,601199,688584,688130,300966,603893,002077" --start 2024-07-01 --end 2026-07-01 --split-date 2025-07-01
```
