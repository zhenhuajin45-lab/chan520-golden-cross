...
def test_three_days_no_new_high_triggers_exit() -> None:
    rows = make_history()
    pos = Position("600000", "测试", "行业", 100, 10, rows[0].date, "entry", 9, 12)
    assert _exit_by_discipline(pos, rows[-1], rows, rows[-1].date, point(rows[-1].date), RiskConfig())
...
def test_time_stop_exit_path() -> None:
    ...
    pos = Position("600000", "测试", "行业", 100, 10, rows[0].date, "entry", 9, 10.2)
    assert _exit_by_discipline(pos, row, rows + [row], row.date, point(row.date), RiskConfig())
